...
static HI_VOID *SAMPLE_IVE_Rfcn_Detect(HI_VOID *pArgs)
{
    ...
    BASE_RELEASE:
        s32Ret = HI_MPI_VPSS_ReleaseChnFrame(s32VpssGrp, 2, &stExtFrmInfo);
...
static HI_VOID *SAMPLE_IVE_GetFrame(HI_VOID *pArgs)
{
    ...
    while (HI_FALSE == s_bIveKcfStopSignal) {
        s32Ret = HI_MPI_VPSS_GetChnFrame(0, 1, &astVideoFrame[0], s32MilliSec);
        if (s32Ret != HI_SUCCESS) {
            continue;
        }
        ...
        if (CNN_GET_FRM_END == s_stIveKcfInfo.enCnnGetFrmStat) {
            ...
            s32Ret = HI_MPI_VPSS_GetChnFrame(0, 2, &astVideoFrame[1], s32MilliSec);
            if (s32Ret != HI_SUCCESS) {
                SAMPLE_IVE_MUTEX_UNLOCK(s_stIveKcfInfo.GetFrmMutex);
                continue;
            }
    ...
    RELEASE0:
        s32Ret = HI_MPI_VPSS_ReleaseChnFrame(0, 1, &astVideoFrame[0]);
        if (s32Ret != HI_SUCCESS) {
